...
class ScannerLexico:
    ...
    def extraer_etiqueta_cerrada(self) -> Token:
        ...
        if palabra not in self.ETIQUETAS:
            self.registrar_error(f"Etiqueta de cierre no válida: {palabra}", inicio_fila, inicio_columna, f"</{palabra}")
    ...
    def scanear(self) -> List[Token]:
        while self.posicion < len(self.texto):
            ...
            # Caracteres inválidos
                ...
                self.registrar_error(f"Carácter no reconocido", self.fila, self.columna - 1, char_invalido)
...
class Parser:
    ...
    def consumir(self, tipo: str) -> Token:
        if not self.esperar(tipo):
            raise Exception(f"Se esperaba {tipo}, se encontró {self.peek().tipo if self.peek() else 'EOF'}")
        return self.advance()
    
    def extraer_tipo_operacion(self, lexema: str) -> str:
        """Extrae el tipo de operación del lexema de etiqueta abierta"""
        # Ejemplos: 
        # <Operacion= SUMA> -> SUMA
        # <Operacion=SUMA> -> SUMA
        # <Numero> -> vacío
        ...
        try:
            ...
            print(f"Error extrayendo tipo de operación: {e}")
            return ""
    
    def parsear(self) -> List[Union[NodoOperacion, float]]:
        """Parsea múltiples operaciones"""
        operaciones = []
    ...
    def parsear_operacion(self) -> NodoOperacion:
        ...
        if not tipo_op or tipo_op == "":
            raise Exception(f"No se pudo extraer tipo de operación de: '{etiqueta_abierta.lexema}'")
        ...
        if tipo_op not in operaciones_validas:
            raise Exception(f"Tipo de operación no válido: '{tipo_op}'")
        ...
        if not operandos and not parametros:
            raise Exception(f"Operación {tipo_op} sin operandos")
...
class Evaluador:
    ...
    def evaluar(self, nodo: Union[NodoOperacion, float, List]) -> Union[float, List[float]]:
        """Evalúa recursivamente el árbol o lista de árboles"""
    ...
    def raiz(self, operandos: List) -> float:
        ...
        if radicando < 0 and int(indice) % 2 == 0:
            raise Exception(f"Raíz {int(indice)} de número negativo")
    ...
    def generar_expresion(self, nodo: Union[NodoOperacion, float, List]) -> str:
        """Genera representación en notación matemática"""
...
class AnalizadorGUI:
    ...
    def crear_widgets(self):
        ...
        self.notebook.add(self.tab_expresion, text="Expresión")
    ...
    def analizar(self):
        ...
        try:
            ...
            print(f"Árbol generado: {self.arbol_actual}")
            ...
            if not expresion or expresion == "":
                raise Exception("No se pudo generar la expresión matemática")
    ...
    def generar_arbol_canvas(self, nodo: Union[NodoOperacion, float], x: float = 250, y: float = 30, 
        ...
        """Dibuja el árbol directamente en Canvas (solo librerías nativas)"""
        if canvas is None:
            return x
    ...
    def mostrar_arbol_visual(self):
        """Dibuja el árbol directamente en Canvas sin librerías externas"""
        try:
            self.canvas_arbol.delete("all")
            ...
            if isinstance(self.arbol_actual, list):
                if len(self.arbol_actual) > 0:
                    ...
                    titulo = f"Mostrando operación {numero_actual} de {total_operaciones}"
    ...
    def exportar_html(self):
        ...
        if not self.arbol_actual or not self.evaluador_actual:
            messagebox.showwarning("Advertencia", "Primero debes analizar el código")
            return
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="section">
                <h2>ℹ️ Información del Análisis</h2>
                <table>
                    <tr>
